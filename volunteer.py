...
def createVolunteer(json_data, token_data): # 大概可以了
	...
	# print(2333)
	# print(token_data)
	if not token_data.get("permission") in [PMS_TEACHER,PMS_MANAGER,PMS_SYSTEM]:
	...
	OP.insert("volName,volDate,volTime,stuMax,nowStuCount,description,status,"
		...
		json_data["description"],VOLUNTEER_WAITING,json_data["inside"],json_data["outside"],json_data["large"],token_data["userid"]))
...
def chooseVolunteer(volId, json_data, token_data):
	pass

@Volunteer.route('/volunteer/joinerList/<int:volId>', methods = ['GET'])
@Deco
def getJoinerList(volId, json_data, token_data): # 这个到底要不要？
	# 所以这个的意思是返回所有审核过了的报名的人吗？
	if not token_data.get("permission")<3:
		return {'type':'ERROR', 'message':"权限不足"}
	ret={"type":"SUCCESS", "message":"获取成功", "result":[]}
...
def auditThought(volId, json_data, token_data): # 大概是过了
	# 判断权限，只有义管会和系统可以审核
	if not token_data["permission"] in [PMS_MANAGER,PMS_SYSTEM]:
		return {'type':'ERROR', 'message':"权限不足"}
	# 判断状态是否可以审核
...
def holidayVolunteer(json_data, token_data):
	...
	print(json_data)
	print(json_data["stuId"],token_data["class"])
	for i in json_data["stuId"]:
		if not token_data["class"]==i//100:
			return {"type":"ERROR", "message":"权限不足：学生列表中有别班学生"}
	stulen=len(json_data["stuId"])
	print(2333)
	print(stulen)
	print(json_data["name"],json_data["date"],json_data["time"],stulen,stulen)
	print(json_data["description"],VOLUNTEER_WAITING,json_data["inside"],json_data["outside"],json_data["large"],token_data["userid"])
	#  先创建一个义工（照搬Create）
	OP.insert("volName,volDate,volTime,stuMax,nowStuCount,description,status,"
		+"volTimeInside,volTimeOutside,volTimeLarge,holderId",
		"volunteer", # 初始把所有学生报进去
		(json_data["name"],json_data["date"],json_data["time"],str(stulen),str(stulen),
		json_data["description"],str(VOLUNTEER_WAITING),json_data["inside"],json_data["outside"],json_data["large"],token_data["userid"]))
	# 获取volId
	fl,r=OP.select("volId","volunteer","volName=%s AND volDate=%s AND volTime=%s",
		(json_data["name"],json_data["date"],json_data["time"]),["id"])
	if not fl: return r # 理论上这个错误不可能发生
	volId=r["id"]
	# 在每个班的表里添加一条记录
	OP.insert("volId,class,stuMax,nowStuCount","class_vol",(volId,token_data["class"],str(stulen),str(stulen)))
	# 给每个学生一条记录
	for i in json_data["stuId"]:
		OP.insert("volId,stuId,status,volTimeInside,volTimeOutside,volTimeLarge,thought",
...
@Deco
def modifyVolunteer(volId, json_data, token_data):
	# 判断权限，教师、义管会和系统（也即可以创建义工的）可以修改
	if not token_data["permission"] in [PMS_TEACHER,PMS_MANAGER,PMS_SYSTEM]:
		return {"type":"ERROR", "message":"权限不足"}
